...
export class CourtDB extends BaseDB<Court> {
   ...
   * Create a new court
   */
  async create(courtData: CourtCreateData): Promise<Court> {
    ...
    return super.create(fullCourtData)
  }
   ...
   */
  async findBySurface(surface: CourtSurface): Promise<Court[]> {
   ...
   */
  async findByLocation(location: string): Promise<Court[]> {
   ...
   */
  async findWithAmenities(amenities: string[]): Promise<Court[]> {
    const courts = await this.findAll()
    return courts.filter(court => 
   ...
   */
  async findWithLighting(): Promise<Court[]> {
   ...
   * Find covered courts
   */
  async findCovered(): Promise<Court[]> {
   ...
   */
  async reserveForMatch(id: string, matchId: string): Promise<Court> {
   ...
   */
  async releaseFromMatch(id: string): Promise<Court> {
    const court = await this.findById(id)
    if (!court) {
      throw new RecordNotFoundError(id, this.entityName)
    }
   ...
   */
  async scheduleNextMatch(id: string, matchId: string): Promise<Court> {
    const court = await this.findById(id)
    if (!court) {
      throw new RecordNotFoundError(id, this.entityName)
    }

    return this.update(id, { nextMatch: matchId })
  }

  /**
   * Put court into maintenance mode
   */
  async setMaintenance(id: string, reason?: string): Promise<Court> {
    const court = await this.findById(id)
    if (!court) {
      throw new RecordNotFoundError(id, this.entityName)
    }
   ...
   */
  async removeMaintenance(id: string): Promise<Court> {
    const court = await this.findById(id)
    if (!court) {
      throw new RecordNotFoundError(id, this.entityName)
    }
   ...
   */
  async updateAmenities(id: string, amenities: string[]): Promise<Court> {
    ...
    return this.update(id, { amenities })
  }

  /**
   * Add amenity to court
   */
  async addAmenity(id: string, amenity: string): Promise<Court> {
    const court = await this.findById(id)
    if (!court) {
      throw new RecordNotFoundError(id, this.entityName)
    }
    ...
    return this.update(id, { amenities: updatedAmenities })
  }

  /**
   * Remove amenity from court
   */
  async removeAmenity(id: string, amenity: string): Promise<Court> {
    const court = await this.findById(id)
    if (!court) {
      throw new RecordNotFoundError(id, this.entityName)
    }
  ...
  async updateDimensions(id: string, dimensions: {
    ...
    return this.update(id, { dimensions })
  }
   ...
   */
  async bulkUpdateStatus(courtIds: string[], status: CourtStatus): Promise<{
    successful: Court[]
    ...
    for (const courtId of courtIds) {
      try {
        const court = await this.updateStatus(courtId, status)
        result.successful.push(court)
      } catch (error) {
        result.failed.push({
          id: courtId,
